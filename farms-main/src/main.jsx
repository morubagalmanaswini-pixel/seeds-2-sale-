...
function App() {
  ...
  return <div className="app-shell">{role === 'farmer' && <FarmerVoiceAssistant language={language} onNavigate={setTab} onAddListing={() => setShowAdd(true)} />}
...
function FarmerVoiceAssistant({ language, onNavigate, onAddListing }) {
  const [listening, setListening] = useState(false);
  const [message, setMessage] = useState('Tap the microphone and ask for a farmer task.');
  const recognitionRef = useRef(null);
  ...
  const handleCommand = (transcript) => { const command = transcript.toLowerCase(); if (/(help|मदद|సహాయం|உதவி|ಸಹಾಯ|സഹായം)/i.test(command)) { speak('Say open machinery, show orders, add produce, or open demand radar.'); return; } if (/(machin|tractor|harvester|sprayer|యంత్ర|मशीन|ట్రాక్టర్|హార్వెస్టర్)/i.test(command)) { speak('Opening machinery sharing.'); onNavigate('machinery'); return; } if (/(order|ऑर्डर|ఆర్డర్|ஆர்டர்|ಆರ್ಡರ್|ഓർഡർ)/i.test(command)) { speak('Opening your orders.'); onNavigate('orders'); return; } if (/(add|list|produce|crop|फसल|जोड़|పంట|జోడ|பயிர்|ಬೆಳೆ|വിള)/i.test(command)) { speak('Opening the produce listing form.'); onAddListing(); return; } if (/(radar|demand|मांग|डिमांड|డిమాండ్|தேவை|ಬೇಡಿಕೆ|ഡിമാൻഡ്)/i.test(command)) { speak('Opening demand radar.'); onNavigate('radar'); return; } speak('I did not understand. Say help for available commands.'); };
  ...
  return <div className={`farmer-voice-assistant ${listening ? 'listening' : ''}`}><div className="farmer-voice-icon"><Mic size={22} /></div><div><span className="panel-kicker">VOICE FARM ASSISTANT</span><h2>Speak in your language</h2><p>{message}</p></div><button type="button" className="voice-assistant-button" onClick={listen} aria-label={listening ? 'Stop voice assistant' : 'Start voice assistant'}>{listening ? <MicOff size={18} /> : <Mic size={18} />}<span>{listening ? 'Listening' : 'Speak'}</span></button></div>;
}
